...
class SimpleConvNet(nn.Module):
    ...
    def forward(self, x):
        ...
        my_conv = conv2d_vector(x, conv_weight=self.conv_layer.weight, conv_bias=self.conv_layer.bias, device = self.device)

        print("TEST!!! conv = ", diff_mse(z_conv, my_conv))
        z_conv = my_conv

        z_pool = F.max_pool2d(z_conv, 2, 2)
        my_pool = pool2d_vector(z_conv, self.device)

        z_pool_reshaped = z_pool.view(-1, 20*12*12)
        print("TEST!!! pool = ", diff_mse(z_pool_reshaped, my_pool))
        z_pool_reshaped = my_pool

        z_fc1 = self.fc_layer1(z_pool_reshaped)
        my_fc1 = fc_layer_vector(z_pool_reshaped, self.fc_layer1.weight, self.fc_layer1.bias, self.device)
        
        print("TEST!!! fc1 = ", diff_mse(z_fc1, my_fc1))
        z_fc1 = my_fc1

        z_relu = F.relu(z_fc1)
        my_relu = relu_vector(z_fc1, self.device)

        print("TEST!!! relu = ", diff_mse(my_relu, z_relu))
        z_relu = my_relu

        z_fc2 = self.fc_layer2(z_relu)
        my_fc2 = fc_layer_vector(z_relu, self.fc_layer2.weight, self.fc_layer2.bias, self.device)
